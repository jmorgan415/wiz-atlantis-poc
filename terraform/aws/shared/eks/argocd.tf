locals {
  argocd-manager-role = "${var.cluster_name}-argocd-manager"
}
...
data "aws_iam_policy_document" "argocd_deployer_assume_role_policy" {
  statement {
    ...
    principals {
      ...
      identifiers = ["arn:aws:iam::${var.account_id}:role/${local.argocd-manager-role}"]
    }
...
resource "aws_iam_role" "argocd_deployer" {
  name               = "${var.cluster_name}-argocd-deployer"
